...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("Upload your insurance policy document", type=["pdf", "txt", "docx"],accept_multiple_files=True)
...
questions=["What are the main reasons for the decline in life insurance penetration in mature markets from 2007 to 2023?",\
           "How does customer dissatisfaction manifest in the life insurance industry, and what are the top CX pain points for policyholders?",\
            "What are the key transformation strategies recommended for life insurers to achieve customer centricity?",\
            "How are best-in-class life insurers using generative AI (Gen AI) to drive business results and enhance customer experience?",\
            "What are the challenges life insurers face in modernizing legacy technology systems, and how does this impact CX?",\
            "Describe the Capgemini blueprint for an efficient, seamless, and customer-centric transformation in life insurance.",\
            "What are the specific onboarding challenges for retail and group life insurance customers?",\
            "How can AI-powered tools improve self-service and claims processes in life insurance?",\
            "What benefits are reported by insurers that have implemented integrated data management and unified customer views?",\
            "What are the skills and talent gaps in the insurance industry’s drive toward greater customer centricity and AI adoption?"]
...
if uploaded_file:
    ...
    #     scores += evaluate_f1(gold, predicted)[2]
    ...
    if chatInput := st.chat_input("Type your question and press Enter...", key="chat_input"):
        ...
        for msg in st.session_state.chat_history[::-1]:
            if msg["type"] == "human":
                ...
                st.markdown(f"Retrieved from {uploaded_file.name} file", unsafe_allow_html=True)